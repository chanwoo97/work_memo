document.getElementById('signupForm').addEventListener('submit', function (e) {
    ...
    // FormDate 클래스, 클래스는 변수, 상수, 기능(함수)들의 모음집
    // new FormData() : 클래스를 이용 할려면, 시동을 걸어야함. 초기화를 함.
    // 생성자를 호출한다고 함.
    // this : 자기 자신 = document.getElementById('signupForm') 가리킴
    // formData 상수에는, 결론, 여러 기능이 들어 있다.
    // 여기서 유저가 입력한 내용이 FormData 클래스에 담아져 있다.
    ...
    // 유저명 부터 표기 해보기
    // output = output + "내용"
    // output += "내용", 같은 의미, 기존 내용에 새로우 내용을 계속 첨부하기.
    // x = x + 1 <=> x += 1

    // 백틱 `, `문자열을 표기함`
    // 1번째 요소, 유저명 가져오기
    // formData.get('username') 의 역할, 이름이 username인 요소의 값을 가져오기
    // ex) ${formData.get('username')} -> jcw로 변환
    output += `출력되는 유저명 : ${formData.get('username')} \n`

    // 2번째 요소, 패스워드 가져오기
    // ex) ${formData.get('password')} -> 1234로 변환
    ...
    // ex) output = 출력되는 유저명 jcw 줄넘김 출력되는 패스워드 : 1234
    ...
    // 5번째 요소, 날짜(생년월일) 가져오기
    output += `출력되는 날짜(생년월일) : ${formData.get('date')} \n`
...
document.getElementById('signupForm').file.addEventListener('change',
    function () {
        // 동작원리
        // 1) 파일선택 2) 선택된 파일 3) 폼 양식의 파일에 변경 감지 이벤트 설정 
        ...
        // 조건, 이미지가 아님에도 화면에 출력을 할려고 하면 안됨.
        // 왜? 이미지만 출력이 가능하니까. 그러면 어떻게 하냐?
        ...
        if (file && file.type.startsWith('image/')) {
            ...
            // 웹브라우저에서 지원 해주는 reader라는 도구를 이용해서 읽기
            // 읽을 때 읽는 방법이 base64()
            // 이미지는(바이너리로 구성 : 01010101010) -> base64 특정의 문자열로 읽기,
            // base64 간단 설명
            ...
            // ex) 윈도우즈, 맥, 리눅스 환경
            // 호환성이 좋음. 그래서, 자주 사용함.
            // 단점) 너무 많은 데이터를 문자열 형식으로 변환작업을 하면 메모리 부족 현상이 일어남. 비추천.
            // 결론) 미디어 서버를 따로 운영을 함.
...
//     output += `출력되는 날짜(생년월일) : ${formData.get('date')} \n`
//     output += `출력되는 이메일 : ${formData.get('email')} \n`
//     output += `출력되는 파일첨부 : ${formData.get('file')} \n`
